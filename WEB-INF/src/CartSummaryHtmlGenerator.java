...
public class CartSummaryHtmlGenerator {
  public static String getCartSummaryPage(Map<CartItem, Integer> userCart) {
    ...
    sb.append(
        "<html><head><title>My Cart</title><link rel=\"stylesheet\""
    ...
    for (Map.Entry<CartItem, Integer> entry : userCart.entrySet()) {
      ...
      sb.append(
          String.format("<img class=\"summary-image\" src=\"%s\">", currentItem.getImgAddress()));
      sb.append(
          String.format(
              "<h3>Name: %s Quantity: %d Total Price: %d</h3>",
              currentItem.getName(), quantity, quantity * currentItem.getPrice()));
    ...
    sb.append(
        "<form><button class=\"cart-button\" type=\"submit\""
            + " formaction=\"/catalog/catalog.html\">Keep Shopping</button></form>");
